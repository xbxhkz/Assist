...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "open_in_vscode": "Open a file or folder in the user's Visual Studio Code editor (optionally at a specific line). Use when the user wants to see or continue editing code in their editor. Confined to the workspace/allowed folders.",
    "launch_app": "Launch an installed app by name, or open a file/URL with its default program.",
    "find_files": "Search the PC for files by name. Returns paths + size. Read the contents with read_file.",
    "list_windows": "List open windows: id, title, process, state. Use before control_window.",
    ...
    "remove_background": "Remove the background from an image, returning a transparent PNG. The image can be an uploaded chat attachment or a filesystem path/filename.",
    "edit_image_prompt": "Edit an image by describing the change in natural language, returning the edited image. The image can be an uploaded chat attachment or a filesystem path/filename.",
    "face_swap": "Swap a face from one image into another. Each image can be an uploaded chat attachment or a filesystem path/filename. Requires accepting the model's license in Settings first.",
    ...
    "run_workflow": "Run a saved workflow by id (or list saved workflows) via the workflow engine.",
    "diagnose_equipment": "Diagnose an industrial image (schematic, fault code/HMI, VFD fault, component/thermal) with a maintenance-expert vision model.",
}
