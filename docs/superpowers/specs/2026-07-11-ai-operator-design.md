...
- **Both consents required to START:** `screen_access_enabled` AND `input_control_enabled` must be ON, or `start` refuses with a message naming the two sidebar toggles. (The 4 input tools — mouse/keyboard/click_element/set_element_text — and `capture_screen` re-check their respective consent on every call, so those are gated regardless. `launch_app` and `control_window` are **not** gated by `input_control_enabled`; they're covered only by the operator's per-action confirmation, which is fail-closed. This means panic-flipping `input_control_enabled` off does not additionally block an in-flight `launch_app`/`control_window` call — the per-action approval is the real gate for those two.)
